...
router.get("/aggregate", async (req, res) => {
  ...
  const aggreatedResult = await orderModel.aggregate([
    { $match: { size: "medium" } }, //filtrar por el tamaño
    { $group: { _id: "$name", total: { $sum: "$quantity" } } }, //agrupar por el nombre
    //A partir de esta linea no tenemos pizzas, va a trabajar con el resultado de group.
    //Una Stage siempre debe ser consciente únicamente de su stage anterior. No puede volver a etapas previas
    { $sort: { total: -1 } }, //ordenar de mayor a menor
    //Ahora lo desafiante es generar el reporte final
    { $group: { _id: "orders", orders: { $push: "$$ROOT" } } }, //$$ROOT vuelve a la raíz
    { $project: { _id: 0, orders: "$orders" } }, // genera un documento con el resultado
    { $merge: { into: "reports" } }, // genera el documento en la base de datos
  ]);
